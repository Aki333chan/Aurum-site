...
Состояние на 26.09.2026: сайт доступен по HTTPS на `aurumgg.ovh` (www перенаправляет на основной домен). Сертификат и пробное продление проверены. Аккаунт `BogOtec` создан с подтверждённой почтой; временный пароль не хранится в репозитории. SMTP ещё не подключён, поэтому регистрация и письма отключены. Привязка Minecraft и каталог гильдий пока не подключены к игровым данным.
